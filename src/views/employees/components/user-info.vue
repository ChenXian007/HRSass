<template>
  <div class="user-info">
    <el-row type="flex" justify="end">
      <el-tooltip placement="top-end" content="打印个人信息">
        <el-button size="mini" type="primary" @click="$router.push(`/employees/print/${userId}?type=personal`)">打印</el-button>
      </el-tooltip>
    </el-row>
    <!-- 个人信息 -->
  ...
  methods: {
    ...
    async getUserDetailById() {
      ...
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        this.$refs.userInfo.fileList = [{ url: this.userInfo.staffPhoto, upload: true }]
      }
    },
    async saveUser() {
      const fileList = this.$refs.userInfo.fileList
      if (fileList.some(item => {
        return item.upload !== true
      })) {
        this.$message('图片还未上传完成')
        return
      }
      this.userInfo.staffPhoto = fileList.length === 1 ? fileList[0].url : ' '
    ...
    async getPersonalDetail() {
      ...
      if (this.formData.staffPhoto && this.formData.staffPhoto.trim()) {
        this.$refs.person.fileList = [{ url: this.formData.staffPhoto, upload: true }]
      }
    },
    async savePersonal() {
      const fileList = this.$refs.person.fileList
      if (fileList.some(item => !item.upload)) {
        this.$message('图片还未上传成功')
        return
      }
      this.formData.staffPhoto = fileList.length === 1 ? fileList[0].url : ' '
